...
(by the DNS naming spec).
*/}}
{{- define "bitwarden-rs.sharedname" }}
{{- $name := default .Chart.Name .Values.nameOverride }}
...
*/}}
{{- define "bitwarden-rs.bitwardenrs-certificate" }}
{{- if (not (empty .Values.ingress.bitwardenrs.certificate)) }}
{{- printf .Values.ingress.bitwardenrs.certificate }}
{{- else }}
{{- printf "%s-bitwardenrs-letsencrypt" (include "bitwarden-rs.fullname" .) }}
{{- end }}
{{- end }}
...
*/}}
{{- define "bitwarden-rs.bitwardenrs-hostname" }}
...
{{- else }}
{{- printf "%s-bitwardenrs" (include "bitwarden-rs.fullname" .) }}
{{- end }}
{{- end }}
{{- end }}
...
*/}}
{{- define "bitwarden-rs.bitwardenrs-base-url" }}
...
{{- else }}
{{- if .Values.ingress.bitwardenrs.enabled }}
{{- $hostname := ((empty (include "bitwarden-rs.bitwardenrs-hostname" .)) | ternary .Values.ingress.bitwardenrs.hostname (include "bitwarden-rs.bitwardenrs-hostname" .)) }}
...
{{- else }}
{{- printf "http://%s" (include "bitwarden-rs.bitwardenrs-hostname" .) }}
{{- end }}
{{- end }}
{{- end }}
