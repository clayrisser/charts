...
helm repo update
helm install appscode/stash-elasticsearch --name=stash-elasticsearch-6.3.0 --version=6.3.0
```
...
```console
helm delete stash-elasticsearch-6.3.0
```
...
```console
helm install --name stash-elasticsearch-6.3.0 --set docker.registry=my-registry appscode/stash-elasticsearch
```
