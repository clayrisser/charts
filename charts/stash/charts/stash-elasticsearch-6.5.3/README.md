...
helm repo update
helm install appscode/stash-elasticsearch --name=stash-elasticsearch-6.5.3 --version=6.5.3
```
...
```console
helm delete stash-elasticsearch-6.5.3
```
...
```console
helm install --name stash-elasticsearch-6.5.3 --set docker.registry=my-registry appscode/stash-elasticsearch
```
