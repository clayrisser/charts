...
{{- if (not (empty .Values.ingress.minio.certificate)) }}
...
{{- if (and .Values.config.minio.hostname (not (empty .Values.config.minio.hostname))) }}
...
{{- if (and .Values.config.minio.baseUrl (not (empty .Values.config.minio.baseUrl))) }}
{{- printf .Values.config.minio.baseUrl }}
{{- else }}
{{- if .Values.ingress.minio.enabled }}
{{- $hostname := ((empty (include "minio.minio-hostname" .)) | ternary .Values.ingress.minio.hostname (include "minio.minio-hostname" .)) }}
