...
prev-artifacts:
	@echo curl -L --globoff -H "PRIVATE-TOKEN: $(GITLAB_TOKEN)" $(CI_SERVER_URL)/api/v4/projects/$(CI_PROJECT_ID)/jobs
	@curl -L --globoff -H "PRIVATE-TOKEN: $(GITLAB_TOKEN)" $(CI_SERVER_URL)/api/v4/projects/$(CI_PROJECT_ID)/jobs
