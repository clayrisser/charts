...
prev-artifacts:
	@echo curl -L $(CI_SERVER_URL)/api/v4/projects/$(CI_PROJECT_ID)/jobs
	@echo jq -r '[.[] | select(.name=="$(CI_JOB_NAME)")][0].id'
	@curl -L $(CI_SERVER_URL)/api/v4/projects/$(CI_PROJECT_ID)/jobs
	@curl -L $(CI_SERVER_URL)/api/v4/projects/$(CI_PROJECT_ID)/jobs | \
